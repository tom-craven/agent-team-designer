...
source under `.opencode/skills/`; the adapters are generated by
`scripts/sync_copilot_adapters.py`, not independently edited copies.
...
- A read-only audit named `.opencode/skills/agent-audit/SKILL.md` as the
  ...
  worktree changes.
- Copilot combines applicable instruction files; current GitHub documentation
  does not define a general precedence order. The two repository instruction
  files were reviewed for conflicts and none were found. The Copilot-specific
  file explicitly directs the agent to `AGENTS.md` for canonical repository
  behavior.
...
- The profile exposes only `read`, `search`, and `web`; it has no `edit`,
  ...
  boundary.
- Frontmatter-shape validation confirmed only `name`, `description`, `model`,
  and `tools` use the repository's supported scalar/list form. The full file is
  below the 30,000-character limit.
- `opencode debug config` loaded the unchanged `AGENTS.md`, `.opencode/skills`,
  and `agent-team-designer` definition with its original model and permissions.
- `git diff --check` completed without errors.
- `python3 scripts/sync_copilot_adapters.py --check` validates generated skill
  adapters, detects OpenCode agent changes that require Copilot translation
  review, and checks the Copilot read-only schema, model format, and prompt
  length limit.

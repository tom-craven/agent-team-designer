...
`scripts/sync_runtime_docs.py`, not independently edited copies.
...
- A read-only audit named `.opencode/skills/agent-audit/SKILL.md` as the
  canonical workflow and identified `AGENTS.md`,
  `.github/copilot-instructions.md`, and
  `docs/agent-team-designer-behavior.md` as governing instructions. The audit
  made no worktree changes.
...
- After the shared-behavior refactor, a live Copilot invocation identified
  `docs/agent-team-designer-behavior.md` as canonical and reported its profile
  as read-only.
- `opencode debug config` loaded `AGENTS.md`,
  `docs/agent-team-designer-behavior.md`, `.opencode/skills`, and the refactored
  `agent-team-designer` definition with its original model and permissions.
- `git diff --check` completed without errors.
- `python3 scripts/sync_runtime_docs.py --check` validates generated adapters,
  canonical behavior references and safety rules, the Copilot read-only tool
  set and schema, model identifier formats, the prompt length limit, and core
  OpenCode discovery and permission settings.
